...
class App(ct.CTk):
    ...
    def __init__(self): 
        ...
        self.frame_left.grid_rowconfigure (9, minsize=20) # empty row as spacing 
        self.frame_left.grid_rowconfigure (11, minsize=10) # empty row with minsize as spacing


        self.lbl_menu = ct.CTkLabel(master=self.frame_left, text="Image Options", font=("Roboto Medium", -16)) # font name and size in px ( , text_font=("Roboto Medium", -16)) 

        self.lbl_menu.grid(row=1, column=0, pady=10, padx=10)

        self.cmd_edge = ct.CTkButton(master=self.frame_left, text="Edge Detection", command=self.edge_detection)

        self.cmd_edge.grid(row=2, column=0, pady=5, padx=20)

        self.cmd_gblur = ct.CTkButton(master=self.frame_left, text="Gaussian Blur", command=self.gaus_blur)

        self.cmd_gblur.grid(row=3, column=0, pady=5, padx=20)

        self.cmd_crop = ct.CTkButton(master=self.frame_left, text="Crop Image", command=self.crop_img)

        self.cmd_crop.grid(row=4, column=0, pady=5, padx=20)

        self.cmd_sharp = ct.CTkButton (master=self.frame_left, text="Sharpen Image", command=self.sharpen_img)

        self.cmd_sharp.grid(row=5, column=0, pady=5, padx=20)

        self.cmd_camera = ct.CTkButton (master=self.frame_left, text="Open Camera", command=self.open_camera)

        self.cmd_camera.grid(row=6, column=0, pady=5, padx=20)

        self.cmd_reset = ct.CTkButton(master=self.frame_left, text="Reset Image", command=self.reset_img) 

        self.cmd_reset.grid(row=7, column=0, pady=5, padx=20)

        self.cmd_exit = ct.CTkButton(master=self.frame_left, text="Exit", fg_color="red", command=self.exit_app)

        self.cmd_exit.grid(row=8, column=0, pady=5, padx=20)

        self.lbl_theme = ct.CTkLabel (master=self.frame_left, text="Window Theme:" , font=("Roboto Medium", -12)) # font name and size in px , text_font=("Roboto Medium", -12)

        self.lbl_theme.grid(row=9, column=0, pady=5, padx=10, sticky="s")

        self.opt_theme = ct.CTkOptionMenu(master= self.frame_left, values=["Light", "Dark", "System"], command = self.set_window_theme)
        ...
        self.img_holder = ct.CTkLabel(master = self.frame_right, text="Image placeholder", bg_color="gray", font=("Roboto Medium", -14)) # text_font=("Roboto Medium", -14)
        
        self.img_holder.grid(row=0, column=0, columnspan=2, pady=15, padx=10, sticky="we")
        self.status_area = st.ScrolledText (master= self.frame_right, width=73, height=7, wrap="word", font = ("Roboto Medium", 10))
        self.status_area.grid(row=9, column=0, pady= 10, padx=10, sticky="nwe") 
        self.status_area.tag_configure("stderr", foreground="#b22222") #b22222
        ...
        # Default values/setup self.filename = ""
    ...
    # Manage edge detection to the loaded image
        global imgtk
        self.clear_status()
        info = "Edge Detection is an image processing technique to find "
    ...
    def gaus_blur(self):
        # Manage applying Gaussian Blur to the loaded image
        global imgtk
        self.clear_status()
        info = "Gaussian filter is used for for image smoothing or "
    ...
    def crop_img(self):
        # Manage cropping to the loaded image
        global imgtk
        self.clear_status()
        ...
        # Set the rectangular portion of the image to be cropped
        hgt, wdt = img.shape[:2]
    ...
    def sharpen_img(self):
        ...
        cv2.waitKey(0) # waits until a key is pressed
        cv2.destroyAllWindows() # destroys the window showing i
    ...
    def saveimg_exit(self):
        ...
        # Look for the occurrence of image file and add file number to the filename. 
        file_ctr = 0
